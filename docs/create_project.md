...
Nuestra escena 3D necesita iluminación para que los objetos sean visibles y tengan un aspecto realista. Para agregar una luz, seleccionamos el nodo raíz ```Node3D``` y luego hacemos clic en el botón "Add Child Node". En la lista de nodos, buscamos y seleccionamos ```DirectionalLight3D```.
...
Es importante que la luz, debe tener las sombras habilitadas para que los objetos en la escena proyecten sombras realistas, lo que mejora significativamente la apariencia visual de nuestro juego. Para habilitar las sombras, seleccionamos el nodo ```DirectionalLight3D``` y en el panel de propiedades, buscamos la sección de "Shadows" y activamos la opción "Enabled". Esto permitirá que los objetos en nuestra escena proyecten sombras basadas en la dirección de la luz, añadiendo profundidad y realismo a nuestro entorno 3D.